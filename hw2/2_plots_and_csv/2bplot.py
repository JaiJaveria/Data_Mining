...
if "time" in figName:
	plt.title("time taken vs size of dataset for kNN Tool for dimension "+str(dim))
	plt.ylabel("Time Duration (in Seconds)")
if "numOp" in figName:
	plt.ylabel("Avg. Number of Operations")
	plt.title("number of Operations vs size of dataset for kNN Tool for dimension "+str(dim))
plt.xlabel("size of dataset (in 1000 entries) ")
# plt.show()
output_loc="./"
