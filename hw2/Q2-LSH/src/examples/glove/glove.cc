...
pair<double, QueryStatistics> evaluate_query_time(
    ...
    const vector<int> &answers, int num_probes, int_fast64_t k) {
  unique_ptr<LSHNearestNeighborQuery<Point>> query_object =
      table->construct_query_object(num_probes);
...
int main(int argc, char **argv) {
  try {
    ...
    int_fast64_t k = std::__cxx11::stoi(argv[1]);
    ...
    */
    ...
    // statistics
    auto tmp = evaluate_query_time(&*table, queries, answers, num_probes, k);
   ...
   elapsed_time = duration_cast<duration<double>>(b - a).count();
    cout << "done" << endl;
    cout << "Running time: " << elapsed_time << endl;
